...
In this blog, I'll explain how to configure Telemetry for gRPC dialin using YDK.
...
Once you've populated the object, it's trivial to apply it to the router using the create method on the CRUDService object:
...
The Subscription is the final piece of the config.  Again, refer to the YANG model to understand the Python class that you should use:

$ pyang -f tree openconfig-telemetry.yang  --tree-path=telemetry-system/subscriptions/persistent/subscription/sensor-profiles
module: openconfig-telemetry
   +--rw telemetry-system
      +--rw subscriptions
         +--rw persistent
